...
type LWControl() =
  ...
  default this.HitTest p =
    (RectangleF(position, size)).Contains(p)
...
type LWCButton() =
    ...
    let mutable text = ""
    let mutable innerColor = Color.DarkBlue
    let mutable borderColor = Color.White
    let mutable pressed = false

    let mutable borderWidth = 4.f

    member this.Pressed with get() = pressed
                        and set(v) = ( pressed <- v; this.PressedChange() )
    member this.Text
        with get() = text
        and set(v) = text <- v
    ...
    member this.PressedChange () =
      if this.Pressed then innerColor <- Color.Cyan
      else innerColor <- Color.DarkBlue
